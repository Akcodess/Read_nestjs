...
import { AgentAOPsStatus, AgentAOPsStatusApis } from "../dtos/widgets/agent.model";
import { Application } from "../dtos/application.model";
import { WIDGET } from "../../common/enums/widget.enum";

export class AgentAOPsStatusProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): AgentAOPsStatus {
        const agentAOPsStatus: AgentAOPsStatus = new AgentAOPsStatus();
        if (widgetConfigs) {
            Object.assign(agentAOPsStatus, widgetConfigs[WIDGET.AgentAOPsStatus]);
            agentAOPsStatus.apis = this.getApiProperties(appProperty);
            agentAOPsStatus.logAppender = this.getLogProperties(appProperty);
        }

        return agentAOPsStatus;
    }

    public static getApiProperties(appProperty: Application): AgentAOPsStatusApis {
        const apis: AgentAOPsStatusApis = new AgentAOPsStatusApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.AgentAOPsStatus}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.AgentAOPsStatus}-%DATE%.log`);
