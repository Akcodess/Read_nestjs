...
import { InboundCallOverview, InboundCallOverviewApis } from "../dtos/widgets/call.model";

export class InboundCallOverviewProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): InboundCallOverview {
        const inboundCallOverview: InboundCallOverview = new InboundCallOverview();
        if (widgetConfigs) {
            Object.assign(inboundCallOverview, widgetConfigs[WIDGET.InboundCallOverview]);
    ...
    public static getApiProperties(appProperty: Application): InboundCallOverviewApis {
        const apis: InboundCallOverviewApis = new InboundCallOverviewApis();

        apis.fetch = `${appProperty.name}/api/fetch`;
        apis.log = `${appProperty.name}/api/${WIDGET.InboundCallOverview}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.InboundCallOverview}-%DATE%.log`);
