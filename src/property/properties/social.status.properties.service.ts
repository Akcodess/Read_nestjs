...
import { SocialStatus, SocialStatusApis } from "../dtos/widgets/social.model";

export class SocialStatusProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): SocialStatus {
        const socialStatus: SocialStatus = new SocialStatus();
        if (widgetConfigs) {
            Object.assign(socialStatus, widgetConfigs[WIDGET.SocialStatus]);
            socialStatus.apis = this.getApiProperties(appProperty);
            socialStatus.logAppender = this.getLogProperties(appProperty);
        }

        return socialStatus;
    }

    public static getApiProperties(appProperty: Application): SocialStatusApis {
        const apis: SocialStatusApis = new SocialStatusApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.SocialStatus}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.SocialStatus}-%DATE%.log`);
