...
import { CallAverageSummary, CallAverageSummaryApis } from "../dtos/widgets/call.model";
...
export class CallAverageSummaryProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): CallAverageSummary {
        const callSummary: CallAverageSummary = new CallAverageSummary();
        if (widgetConfigs) {
            Object.assign(callSummary, widgetConfigs[WIDGET.CallAverageSummary]);
    ...
    public static getApiProperties(appProperty: Application): CallAverageSummaryApis {
        const apis: CallAverageSummaryApis = new CallAverageSummaryApis();

        apis.fetch = `${appProperty.name}/api/fetch`;
        apis.log = `${appProperty.name}/api/${WIDGET.CallAverageSummary}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.CallAverageSummary}-%DATE%.log`);
