...
import { WebFormSummary, WebFormSummaryApis } from "../dtos/widgets/webform.model";

export class WebFormSummaryProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): WebFormSummary {
        const webFormSummary: WebFormSummary = new WebFormSummary();
        if (widgetConfigs) {
            Object.assign(webFormSummary, widgetConfigs[WIDGET.WebFormSummary]);
            webFormSummary.apis = this.getApiProperties(appProperty);
            webFormSummary.logAppender = this.getLogProperties(appProperty);
        }

        return webFormSummary;
    }

    public static getApiProperties(appProperty: Application): WebFormSummaryApis {
        const apis: WebFormSummaryApis = new WebFormSummaryApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.WebFormSummary}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.WebFormSummary}-%DATE%.log`);
