...
import { UserDispositionGrid, UserDispositionGridApis } from "../dtos/widgets/disposition.model";

export class UserDispositionGridProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, property: Property): UserDispositionGrid {
        const userDispositionGrid: UserDispositionGrid = new UserDispositionGrid();
        if (widgetConfigs) {
            Object.assign(userDispositionGrid, widgetConfigs[WIDGET.UserDispositionGrid]);
            userDispositionGrid.apis = this.getApiProperties(property);
            userDispositionGrid.logAppender = this.getLogProperties(property)
        }

        return userDispositionGrid;
    }

    private static getApiProperties(property: Property): UserDispositionGridApis {
        const apis: UserDispositionGridApis = new UserDispositionGridApis();

        apis.fetch = `${property.application.name}/api/fetch`;
        apis.log = `${property.application.name}/api/${WIDGET.UserDispositionGrid}/log`;
    ...
    private static getRollingFileLogProperties(property: Property): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.UserDispositionGrid}-%DATE%.log`);
