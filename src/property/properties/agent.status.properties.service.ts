...
import { AgentStatus, AgentStatusApis } from "../dtos/widgets/agent.model";
import { Application } from "../dtos/application.model";
import { WIDGET } from "../../common/enums/widget.enum";

export class AgentStatusProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): AgentStatus {
        const agentStatus: AgentStatus = new AgentStatus();
        if (widgetConfigs) {
            Object.assign(agentStatus, widgetConfigs[WIDGET.AgentStatus]);
            agentStatus.apis = this.getApiProperties(appProperty);
            agentStatus.logAppender = this.getLogProperties(appProperty);
        }

        return agentStatus;
    }

    public static getApiProperties(appProperty: Application): AgentStatusApis {
        const apis: AgentStatusApis = new AgentStatusApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.AgentStatus}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.AgentStatus}-%DATE%.log`);
