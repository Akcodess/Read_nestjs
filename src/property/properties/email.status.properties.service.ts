...
import { EmailStatus, EmailStatusApis } from "../dtos/widgets/email.model";

export class EmailStatusProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): EmailStatus {
        const emailStatus: EmailStatus = new EmailStatus();
        if (widgetConfigs) {
            Object.assign(emailStatus, widgetConfigs[WIDGET.EmailStatus]);
            emailStatus.apis = this.getApiProperties(appProperty);
            emailStatus.logAppender = this.getLogProperties(appProperty);
        }

        return emailStatus;
    }

    public static getApiProperties(appProperty: Application): EmailStatusApis {
        const apis: EmailStatusApis = new EmailStatusApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.EmailStatus}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.EmailStatus}-%DATE%.log`);
