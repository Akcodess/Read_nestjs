...
import { ChatSummary, ChatSummaryApis } from "../dtos/widgets/chat.model";

export class ChatSummaryProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): ChatSummary {
        const chatSummary: ChatSummary = new ChatSummary();
        if (widgetConfigs) {
            Object.assign(chatSummary, widgetConfigs[WIDGET.ChatSummary]);
            chatSummary.apis = this.getApiProperties(appProperty);
            chatSummary.logAppender = this.getLogProperties(appProperty);
        }

        return chatSummary;
    }

    public static getApiProperties(appProperty: Application): ChatSummaryApis {
        const apis: ChatSummaryApis = new ChatSummaryApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.ChatSummary}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.ChatSummary}-%DATE%.log`);
