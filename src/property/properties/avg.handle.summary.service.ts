...
import { AverageHandleSummary, AverageHandleSummaryApis } from "../dtos/widgets/avg.model";
...
export class AverageHandleSummaryProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): AverageHandleSummary {
        const averageHandleSummary: AverageHandleSummary = new AverageHandleSummary();
        if (widgetConfigs) {
            Object.assign(averageHandleSummary, widgetConfigs[WIDGET.AverageHandleSummary]);
    ...
    public static getApiProperties(appProperty: Application): AverageHandleSummaryApis {
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.AverageHandleSummary}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.AverageHandleSummary}-%DATE%.log`);
