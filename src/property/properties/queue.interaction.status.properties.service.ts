...
import { QueueInteractionStatus, QueueInteractionStatusApis } from "../dtos/widgets/queue.model";

export class QueueInteractionStatusProperties {

    private LOG_FILE_NAME: string = `queue_interaction_status.log`;

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): QueueInteractionStatus {
        const queueStatus: QueueInteractionStatus = new QueueInteractionStatus();
        if (widgetConfigs) {
            Object.assign(queueStatus, widgetConfigs[WIDGET.QueueInteractionStatus]);
    ...
    public static getApiProperties(appProperty: Application): QueueInteractionStatusApis {
        const apis: QueueInteractionStatusApis = new QueueInteractionStatusApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.QueueInteractionStatus}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.QueueInteractionStatus}-%DATE%.log`);
