...
import { CallStatus, CallStatusApis } from "../dtos/widgets/call.model";

export class CallStatusProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): CallStatus {
        const callStatus: CallStatus = new CallStatus();
        if (widgetConfigs) {
            Object.assign(callStatus, widgetConfigs[WIDGET.CallStatus]);
            callStatus.apis = this.getApiProperties(appProperty);
            callStatus.logAppender = this.getLogProperties(appProperty);
        }

        return callStatus;
    }

    public static getApiProperties(appProperty: Application): CallStatusApis {
        const apis: CallStatusApis = new CallStatusApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.CallStatus}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.CallStatus}-%DATE%.log`);
