...
import { QueueAgentStatus, QueueAgentStatusApis } from "../dtos/widgets/queue.model";

export class QueueAgentStatusProperties {

    private LOG_FILE_NAME: string = `queue_agent_status.log`;

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): QueueAgentStatus {
        const queueStatus: QueueAgentStatus = new QueueAgentStatus();
        if (widgetConfigs) {
            Object.assign(queueStatus, widgetConfigs[WIDGET.QueueAgentStatus]);
    ...
    public static getApiProperties(appProperty: Application): QueueAgentStatusApis {
        const apis: QueueAgentStatusApis = new QueueAgentStatusApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.QueueAgentStatus}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.QueueAgentStatus}-%DATE%.log`);
