...
import { VisitSummary, VisitSummaryApis } from "../dtos/widgets/visit.model";

export class VisitSummaryProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): VisitSummary {
        const visitSummary: VisitSummary = new VisitSummary();
        if (widgetConfigs) {
            Object.assign(visitSummary, widgetConfigs[WIDGET.VisitSummary]);
            visitSummary.apis = this.getApiProperties(appProperty);
            visitSummary.logAppender = this.getLogProperties(appProperty);
        }

        return visitSummary;
    }

    public static getApiProperties(appProperty: Application): VisitSummaryApis {
        const apis: VisitSummaryApis = new VisitSummaryApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.VisitSummary}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.VisitSummary}-%DATE%.log`);
