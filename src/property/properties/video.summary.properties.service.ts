...
import { VideoSummary, VideoSummaryApis } from "../dtos/widgets/video.model";

export class VideoSummaryProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): VideoSummary {
        const videoSummary: VideoSummary = new VideoSummary();
        if (widgetConfigs) {
            Object.assign(videoSummary, widgetConfigs[WIDGET.VideoSummary]);
            videoSummary.apis = this.getApiProperties(appProperty);
            videoSummary.logAppender = this.getLogProperties(appProperty);
        }

        return videoSummary;
    }

    public static getApiProperties(appProperty: Application): VideoSummaryApis {
        const apis: VideoSummaryApis = new VideoSummaryApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.VideoSummary}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.VideoSummary}-%DATE%.log`);
