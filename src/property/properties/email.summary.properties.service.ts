...
import { EmailSummary, EmailSummaryApis } from "../dtos/widgets/email.model";

export class EmailSummaryProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): EmailSummary {
        const emailSummary: EmailSummary = new EmailSummary();
        if (widgetConfigs) {
            Object.assign(emailSummary, widgetConfigs[WIDGET.EmailSummary]);
            emailSummary.apis = this.getApiProperties(appProperty);
            emailSummary.logAppender = this.getLogProperties(appProperty);
        }

        return emailSummary;
    }

    public static getApiProperties(appProperty: Application): EmailSummaryApis {
        const apis: EmailSummaryApis = new EmailSummaryApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.EmailSummary}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.EmailSummary}-%DATE%.log`);
