...
import { CampaignSessionSummary, CampaignSessionSummaryApis } from "../dtos/widgets/campaign.model";

export class CampaignSessionSummaryProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): CampaignSessionSummary {
        const campaignSummary: CampaignSessionSummary = new CampaignSessionSummary();
        if (widgetConfigs) {
            Object.assign(campaignSummary, widgetConfigs[WIDGET.CampaignSessionSummary]);
    ...
    public static getApiProperties(appProperty: Application): CampaignSessionSummaryApis {
        const apis: CampaignSessionSummaryApis = new CampaignSessionSummaryApis();
        apis.fetch = `${appProperty.name}/api/fetch`;
        apis.log = `${appProperty.name}/api/${WIDGET.CampaignSessionSummary}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.CampaignSessionSummary}-%DATE%.log`);
