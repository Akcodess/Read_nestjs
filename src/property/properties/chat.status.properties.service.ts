...
import { ChatStatus, ChatStatusApis } from "../dtos/widgets/chat.model";

export class ChatStatusProperties {

    constructor() { }

    public static getProperties(widgetConfigs: any, appProperty: Application): ChatStatus {
        const chatStatus: ChatStatus = new ChatStatus();
        if (widgetConfigs) {
            Object.assign(chatStatus, widgetConfigs[WIDGET.ChatStatus]);
            chatStatus.apis = this.getApiProperties(appProperty);
            chatStatus.logAppender = this.getLogProperties(appProperty);
        }

        return chatStatus;
    }

    public static getApiProperties(appProperty: Application): ChatStatusApis {
        const apis: ChatStatusApis = new ChatStatusApis();
        ...
        apis.log = `${appProperty.name}/api/${WIDGET.ChatStatus}/log`;
    ...
    public static getRollingFileLogProperties(appProperty: Application): RollingFileAppender {
        ...
        rollingFileAppender.filename = path.join(process.cwd(), `logs/${WIDGET.ChatStatus}-%DATE%.log`);
